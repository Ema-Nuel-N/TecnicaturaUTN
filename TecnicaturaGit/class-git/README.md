...
```sh
cd tecnicatura #cd Sirve para acceder a la carpeta

cd class-git

ls

touch historia.txt #touch sirve para crear un archivo
...
git status #Mostrar que esta en el staging area o no

git add . #Agregar al area staging
...
cd .. #cd .. sirve para volver a la carpeta anterior
...
```sh
git branch cambios #Creamos nueva rama
...
git branch -D cambios #Forzar borrado de rama
...
git add reset_file.txt #Agregamos al staging area

git add . #agregamos todo al staging area

git commit -m"Iniciando el primer commit" #Hacemos commit
...
>Git nunca borra nada, a menos que nosotros se lo indiquemos. Cuando usamos los comandos 
```sh
git merge o git checkout
``` 
>estamos cambiando de rama o creando un nuevo commit, no borrando ramas ni commits (recuerda que puedes borrar commits con git reset y ramas con git branch -d).
...
>Si usamos GitHub solo con usuario y contraseña, si un día perdemos nuestra PC perdemos todo: contraseñas, proyectos y acceso. Por eso es mejor usar llaves públicas y privadas, que ofrecen una capa de seguridad más fuerte y evitan tener que ingresar usuario y contraseña cada vez.

>En nuestra máquina debemos crear una llave privada y una llave pública. La llave pública se envía a GitHub y se asocia con nuestra llave privada local. En vez de conectarnos por HTTPS, nos conectamos por SSH.

>En la primera conexión, GitHub reconocerá la llave pública que enviaste y responderá con su propia llave pública cifrada para que solo tu llave privada la pueda usar. También puedes proteger tu llave privada con una contraseña adicional.

>Las llaves SSH no son por repositorio ni por proyecto; son por persona y por máquina.

## Objetivos

- Entender cómo funciona la autenticación SSH.
- Generar llaves SSH en local.
- Configurar el agente SSH.
- Agregar la llave privada al agente.
- Revisar el uso básico de 2FA.
...
ssh-add ~/.ssh/id_gd456123 # Añadimos la ruta, no la .pub que es la pública.

## ¿Qué es una ruta?

Una ruta es la ubicación de un archivo o carpeta en tu sistema. Por ejemplo: `~/.ssh/id_rsa`.

## Cómo generar tus llaves SSH

Recuerda que es muy buena idea proteger tu llave privada con una contraseña. Esto es para que controles que has hecho todo bien:
```
...
Si usas una versión de macOS superior a Mac Sierra (10.12), crea o modifica un archivo `config` en tu carpeta de usuario con el siguiente contenido:
...
Luego agrega tu llave al agente:
...
El 2FA protege tu cuenta si pierdes el celular u ordenador. Debes tener un respaldo para mantener el acceso.

Pasos para activar 2FA en GitHub:

1. Haz clic en tu perfil, arriba a la derecha.
2. Selecciona `Settings`.
3. Ve a `Password and authentication`.
4. Activa `GitHub Mobile`.
5. Activa `Authenticator app`.

Puedes usar una app como Twilio Authy para generar códigos con QR.

La tarea de hoy es agregar esta clase al `README.md` con Markdown y luego hacer el commit correspondiente dentro de la carpeta `class-git`.

Revisar y ejecutar cada comando como práctica.
